...
from dynex import BQM as DynexBQM, DynexSampler as DynexSamplerV2, DynexConfig, ComputeBackend
...
class DYNEX_QRBM:
    ...
    def sample_opposite_layer_pyqubo(self, v, layer, weights, opposite_layer):
        ...
        dnxmodel = DynexBQM(bqm, config=config)
        dnxsampler = DynexSamplerV2(dnxmodel, logging=self.logging, description=self.description, config=config)
    ...
    def sample_opposite_layer_pyqubo_batch(self, v_batch, layer, weights, opposite_layer):
        ...
        if not bqm.variables:
            return [[0] * len(opposite_layer)] * len(v_batch)
    ...
    def infer(self, visible: npt.NDArray):
        ...
        hidden = (prob_hidden > self.rnd.random(prob_hidden.shape)).astype(np.uint8)
    ...
    def generate_visible_layer(self, hidden: npt.NDArray):
        ...
        visible = (prob_visible > self.rnd.random(prob_visible.shape)).astype(np.uint8)
