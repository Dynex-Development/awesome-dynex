...
class dnx(nn.Module):
    ...
    def _fit_batch(self, data: npt.NDArray) -> float:
        ...
        visible_data = (data > self.rnd.random(data.shape)).astype(np.uint8)
...
class dnx_experimental(nn.Module):
    ...
    def sample_opposite_layer_pyqubo(self, v, layer, weights, opposite_layer):
        ...
        dnxmodel = BQM(bqm, logging=False)
        config = DynexConfig(compute_backend="cpu")
        dnxsampler = DynexSampler(dnxmodel, logging=self.logging,
                                        description="PyTorch QRBM", config=config)
        dnxsampler.sample(num_reads=self.num_reads, annealing_time=self.annealing_time)
