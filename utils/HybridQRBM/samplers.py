...
__copyright__ = "Dynex Developers, 2026"
...
class Sampler:
    ...
    def infer(self, visible: npt.NDArray):
        ...
        hidden = (prob_hidden > self.rbm.rnd.random(prob_hidden.shape)).astype(np.uint8)
    ...
    def generate(self, hidden: npt.NDArray):
        ...
        visible = (prob_visible > self.rbm.rnd.random(prob_visible.shape)).astype(np.uint8)
    ...
    def predict(self, features: npt.NDArray, num_particles: int = 10,
        ...
        Parameters
        ----------
        features : numpy.ndarray[np.uint8]
        ...
        Returns
        -------
        labels : numpy.ndarray[np.float64]
...
class PersistentContrastiveDivergenceSampler(Sampler):
    ...
    def sample(self, visible, **kwargs) -> tuple:
        ...
        if self.visible is None:
            self.visible = (self.rbm.rnd.random(visible.shape) > 0.5).astype(np.uint8)
...
class NaiveSampler(Sampler):
    ...
    def sample(self, visible, **kwargs):
        ...
        visible, prob_visible, hidden, prob_hidden : tuple
            See gibbs_updates() method for details.
        """
        visible = (self.rbm.rnd.random(visible.shape) > 0.5).astype(np.uint8)
        return self.gibbs_updates(visible)


class DynexSampler(Sampler):

    def __init__(self, num_reads=100, annealing_time=300, minimum_stepsize=0.00000006, logging=True,
                 debugging=False, num_gibbs_updates=0, config: DynexConfig = None, **kwargs):
        ...
        self.config = config or DynexConfig()
    ...
    def _sample_dynex(self, qubo, num_particles):
        ...
        model = dynex.BQM(bqm)
        dnxsampler = dynex.DynexSampler(model, logging=self.logging,
                                        description='PyTorch DNX Layer', config=self.config)
        sampleset = dnxsampler.sample(num_reads=self.num_reads, annealing_time=self.annealing_time,
                                      debugging=self.debugging, minimum_stepsize=self.minimum_stepsize)
        if len(sampleset) > 0:
            ...
            self.rbm.logger.info("Dynex Platform: invalid sample returned")
