...
from examples.utils.dynex_scikit_plugin.utilities import corrcoef
...
class SelectFromQuadraticModel(SelectorMixin, BaseEstimator):
    ...
    def fit(
        ...
        model = BQM(bqm)
        sampler = DynexSampler(model, description="Dynex SciKit Plugin", config=DynexConfig(compute_backend="cpu"))
