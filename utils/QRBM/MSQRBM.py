...
class MSQRBM:
    ...
    def train(self, training_data, len_x=1, len_y=1, epochs=50, lr=0.1, lr_decay=0.1, epoch_drop=None, momentum=0,
              batch_size=None):
        """
        ...
        for epoch in self.tqdm(range(epochs)):
            ...
            # better MSE?
            # normalized by dividing image size
            mse = np.sum((np.array(v) - np.array(sample_output)) ** 2) / 784
