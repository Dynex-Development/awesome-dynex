...
class QSVM_Layer(nn.Module):
    """
        ...
        .. code-block:: Pyton
            ...
            class QSVMModel(nn.Module):
                def __init__(self, B,K,C,gamma,xi,dataset,train_percent,spl,num_reads,annealing_time):
                    super().__init__();
                    # Dynex Neuromporphic layer
                    self.dnxlayer = QSVM_Layer(B,K,C,gamma,xi,dataset,train_percent,spl,num_reads,annealing_time);
            ...
            model = QSVMModel(B,K,C,gamma,xi,bank_dataset,train_percent,spl,num_reads,annealing_time)
    ...
    def __init__(self, B: int, K: int, C: int, gamma: int, xi: float, dataset, train_percent, sampler_type,
    ...
    def train(self, save_model=True, save_path='./models'):
            ...
            for m in range(self.N):
                for k in range(self.K):
                    for j in range(self.K):
                        Q_tilde[(self.K * n + k, self.K * m + j)] = 0.5 * (self.B ** (k + j)) * self.t[n] * self.t[
                            m] * (self.kernel(self.data[n], self.data[m]) + self.xi) - (
                                                                                self.delta(n, m) * self.delta(k, j) * (
                                                                                    self.B ** k))
        ...
        if (self.sampler_type == 'DNX'):
            response = sample_qubo(qubo, description='Dynex QSVM (PyTorch)',
        ...
        self.b = sum([self.alpha[n] * (self.C - self.alpha[n]) * (
                    self.t[n] - (sum([self.alpha[m] * self.t[m] * self.kernel(self.data[m], self.data[n])
                                      for m in range(self.N)]))) for n in range(self.N)]) / sum(
            [self.alpha[n] * (self.C - self.alpha[n]) for n in range(self.N)])
    ...
    def save_model(self, path):
        ...
        """
        torch.save({'B': self.B, 'K': self.K, 'C': self.C, 'gamma': self.gamma, 'xi': self.xi, 'alpha': self.alpha,
                    'b': self.b}, path)
